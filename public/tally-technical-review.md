...
### 11.9 Batch rollback for statement imports — SHIPPED (v1.53.0)

**Problem**: "Add all as expense" and per-row resolve create multiple `Expense`/`Transfer` records from one import, but nothing reversed an import as a unit if it turned out wrong (wrong account selected, garbled extraction) — the existing "delete import" deliberately keeps already-logged records, so undoing a bad import meant manually finding and deleting each one by hand.
