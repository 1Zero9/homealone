import type { PresetItem } from '../types/expense';

export const PRESETS: PresetItem[] = [
  // 1. Education, Schooling & College
  {
    id: 'college-tuition',
    name: 'College / University Tuition',
    defaultAmount: 250.00,
    category: 'education',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'GraduationCap',
    description: 'Higher education student contribution / tuition fee installment',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'school-fees-transport',
    name: 'School Fees & Bus Transport',
    defaultAmount: 120.00,
    category: 'education',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'BookOpen',
    description: 'Monthly school fee contribution, bus pass & learning materials',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'school-lunches',
    name: 'School Lunches & Meals',
    defaultAmount: 70.00,
    category: 'education',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Utensils',
    description: 'School canteen & hot lunch programme subscription',
    defaultPaymentMethod: 'Debit Card',
    popular: true,
  },
  {
    id: 'music-lessons',
    name: 'Music & Instrument Tuition',
    defaultAmount: 80.00,
    category: 'education',
    defaultCycle: 'monthly',
    color: '#202124',
    icon: 'Music',
    description: 'Weekly piano, guitar or violin lessons',
    defaultPaymentMethod: 'Direct Debit',
    popular: false,
  },

  // 2. Sports & Activities
  {
    id: 'youth-sports-club',
    name: 'Youth Sports Club & Coaching',
    defaultAmount: 45.00,
    category: 'lifestyle',
    defaultCycle: 'monthly',
    color: '#202124',
    icon: 'Trophy',
    description: 'Football, GAA, rugby, or basketball club coaching & membership',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'swimming-lessons',
    name: 'Swimming Lessons & Pool Pass',
    defaultAmount: 40.00,
    category: 'lifestyle',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Waves',
    description: 'Weekly swimming academy lessons and public pool family pass',
    defaultPaymentMethod: 'Direct Debit',
    popular: true,
  },
  {
    id: 'tennis-club',
    name: 'Tennis & Racket Club',
    defaultAmount: 55.00,
    category: 'lifestyle',
    defaultCycle: 'monthly',
    color: '#202124',
    icon: 'Activity',
    description: 'Tennis court booking access, league fees & coaching',
    defaultPaymentMethod: 'Direct Debit',
    popular: false,
  },
  {
    id: 'gym-membership',
    name: 'Gym & Fitness Membership',
    defaultAmount: 34.99,
    category: 'lifestyle',
    defaultCycle: 'monthly',
    color: '#676B73',
    icon: 'Dumbbell',
    description: 'Full gym and fitness facility access',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },

  // 3. Streaming & Media
  {
    id: 'netflix',
    name: 'Netflix (Standard)',
    defaultAmount: 13.99,
    category: 'entertainment',
    defaultCycle: 'monthly',
    color: '#F04E3E',
    icon: 'Tv',
    description: '1080p streaming on 2 devices simultaneously, ad-free',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'netflix-premium',
    name: 'Netflix (Premium 4K)',
    defaultAmount: 19.99,
    category: 'entertainment',
    defaultCycle: 'monthly',
    color: '#F04E3E',
    icon: 'Tv',
    description: '4K Ultra HD + HDR, spatial audio, 4 devices simultaneously',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'spotify',
    name: 'Spotify Premium (Family)',
    defaultAmount: 17.99,
    category: 'entertainment',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Music',
    description: 'Up to 6 ad-free accounts for family members living together',
    defaultPaymentMethod: 'Visa Debit',
    popular: true,
  },
  {
    id: 'apple-tv',
    name: 'Apple TV+',
    defaultAmount: 9.99,
    category: 'entertainment',
    defaultCycle: 'monthly',
    color: '#202124',
    icon: 'Film',
    description: 'Apple Original series, movies, and documentaries in 4K HDR',
    defaultPaymentMethod: 'Apple Pay',
    popular: true,
  },
  {
    id: 'disney-plus',
    name: 'Disney+ (Standard)',
    defaultAmount: 9.99,
    category: 'entertainment',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Tv',
    description: 'Disney, Pixar, Marvel, Star Wars, and National Geographic',
    defaultPaymentMethod: 'Mastercard',
    popular: true,
  },
  {
    id: 'youtube-premium',
    name: 'YouTube Premium',
    defaultAmount: 13.99,
    category: 'entertainment',
    defaultCycle: 'monthly',
    color: '#F04E3E',
    icon: 'PlayCircle',
    description: 'Ad-free YouTube & YouTube Music with background playback',
    defaultPaymentMethod: 'Visa Debit',
    popular: true,
  },

  // 4. AI & Tech Tools
  {
    id: 'chatgpt-plus',
    name: 'ChatGPT Plus (OpenAI)',
    defaultAmount: 22.99,
    category: 'ai-tech',
    defaultCycle: 'monthly',
    color: '#1a3299',
    icon: 'Bot',
    description: 'GPT-4o, o1 reasoning models, DALL·E generation, canvas, browsing',
    defaultPaymentMethod: 'Credit Card',
    popular: true,
  },
  {
    id: 'claude-pro',
    name: 'Claude Pro (Anthropic)',
    defaultAmount: 22.99,
    category: 'ai-tech',
    defaultCycle: 'monthly',
    color: '#202124',
    icon: 'Sparkles',
    description: 'Claude 3.5 Sonnet, long context analysis, Projects, Artifacts',
    defaultPaymentMethod: 'Credit Card',
    popular: true,
  },
  {
    id: 'cursor-pro',
    name: 'Cursor Pro',
    defaultAmount: 20.00,
    category: 'ai-tech',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Code2',
    description: 'AI code editor with Claude 3.5 Sonnet, Composer, codebase indexing',
    defaultPaymentMethod: 'Credit Card',
    popular: true,
  },
  {
    id: 'midjourney',
    name: 'Midjourney Standard',
    defaultAmount: 28.00,
    category: 'ai-tech',
    defaultCycle: 'monthly',
    color: '#202124',
    icon: 'Image',
    description: '15h fast GPU generations per month, relaxed GPU image creation',
    defaultPaymentMethod: 'Credit Card',
    popular: true,
  },
  {
    id: 'github-copilot',
    name: 'GitHub Copilot',
    defaultAmount: 10.00,
    category: 'ai-tech',
    defaultCycle: 'monthly',
    color: '#202124',
    icon: 'Terminal',
    description: 'AI pair programmer with code completions and chat assistance',
    defaultPaymentMethod: 'Direct Debit',
    popular: true,
  },
  {
    id: 'icloud-storage',
    name: 'iCloud+ (200GB)',
    defaultAmount: 2.99,
    category: 'ai-tech',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Cloud',
    description: 'Apple cloud storage, photo library backup, iCloud Private Relay',
    defaultPaymentMethod: 'Apple Pay',
    popular: true,
  },

  // 5. Home Utilities
  {
    id: 'electricity-gas',
    name: 'Electricity & Gas Dual Fuel',
    defaultAmount: 155.00,
    category: 'utilities',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Zap',
    description: 'Home power and heating regular monthly direct debit',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'broadband-fiber',
    name: 'High-Speed Fibre Broadband',
    defaultAmount: 39.99,
    category: 'utilities',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Wifi',
    description: 'Gigabit fibre internet connection with Wi-Fi router',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'water-utility',
    name: 'Water & Sewerage Bill',
    defaultAmount: 32.50,
    category: 'utilities',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Droplets',
    description: 'Clean water supply and wastewater management',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'mobile-contract',
    name: 'Mobile Phone Plan (5G Unlimited)',
    defaultAmount: 19.99,
    category: 'utilities',
    defaultCycle: 'monthly',
    color: '#3155D9',
    icon: 'Smartphone',
    description: 'Unlimited 5G data, EU roaming, national calls and SMS',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },

  // 6. Housing & Living
  {
    id: 'municipal-tax',
    name: 'Municipal & Property Tax',
    defaultAmount: 185.00,
    category: 'housing',
    defaultCycle: 'monthly',
    color: '#202124',
    icon: 'Building2',
    description: 'Local municipal tax and waste management rate',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'home-insurance',
    name: 'Home & Contents Insurance',
    defaultAmount: 26.00,
    category: 'housing',
    defaultCycle: 'monthly',
    color: '#202124',
    icon: 'ShieldCheck',
    description: 'Property structure and personal contents insurance policy',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },

  // 7. Mortgage, Loans & Big Purchases
  {
    id: 'mortgage-repayment',
    name: 'Mortgage Repayment',
    defaultAmount: 1450.00,
    category: 'big-ticket',
    defaultCycle: 'monthly',
    color: '#B45309',
    icon: 'Landmark',
    description: 'Monthly home mortgage capital & interest repayment',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'car-loan-repayment',
    name: 'Car Loan Repayment',
    defaultAmount: 320.00,
    category: 'big-ticket',
    defaultCycle: 'monthly',
    color: '#B45309',
    icon: 'Car',
    description: 'Monthly car finance / auto loan installment',
    defaultPaymentMethod: 'Direct Debit',
    popular: true,
  },
  {
    id: 'personal-loan-repayment',
    name: 'Personal Loan Repayment',
    defaultAmount: 180.00,
    category: 'big-ticket',
    defaultCycle: 'monthly',
    color: '#B45309',
    icon: 'Banknote',
    description: 'Monthly repayment on a personal or credit union loan',
    defaultPaymentMethod: 'Direct Debit',
    popular: false,
  },
  {
    id: 'holiday-fund',
    name: 'Holiday & Travel Fund',
    defaultAmount: 200.00,
    category: 'big-ticket',
    defaultCycle: 'monthly',
    color: '#B45309',
    icon: 'Plane',
    description: 'Regular set-aside towards annual family holiday or big trip',
    defaultPaymentMethod: 'Standing Order',
    popular: true,
  },

  // 8. Insurance, Motor Tax & NCT
  {
    id: 'car-insurance',
    name: 'Car Insurance',
    defaultAmount: 650.00,
    category: 'insurance',
    defaultCycle: 'annual',
    color: '#0E7490',
    icon: 'Car',
    description: 'Annual comprehensive or third-party motor insurance premium',
    defaultPaymentMethod: 'Credit Card',
    popular: true,
  },
  {
    id: 'motor-tax',
    name: 'Motor Tax',
    defaultAmount: 200.00,
    category: 'insurance',
    defaultCycle: 'annual',
    color: '#0E7490',
    icon: 'Car',
    description: 'Annual vehicle motor tax renewal',
    defaultPaymentMethod: 'Debit Card',
    popular: true,
  },
  {
    id: 'nct-test',
    name: 'NCT Test Fee',
    defaultAmount: 55.00,
    category: 'insurance',
    defaultCycle: 'annual',
    color: '#0E7490',
    icon: 'ClipboardCheck',
    description: 'National Car Test roadworthiness inspection fee',
    defaultPaymentMethod: 'Debit Card',
    popular: true,
  },
  {
    id: 'life-insurance',
    name: 'Life Insurance',
    defaultAmount: 45.00,
    category: 'insurance',
    defaultCycle: 'monthly',
    color: '#0E7490',
    icon: 'ShieldCheck',
    description: 'Term or whole-of-life insurance policy premium',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
  {
    id: 'health-insurance',
    name: 'Health Insurance',
    defaultAmount: 120.00,
    category: 'insurance',
    defaultCycle: 'monthly',
    color: '#0E7490',
    icon: 'HeartPulse',
    description: 'Private health insurance policy premium',
    defaultPaymentMethod: 'SEPA Direct Debit',
    popular: true,
  },
];
