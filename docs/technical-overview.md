...
- **`src/lib/`** — server-only logic: `auth.ts`, `crypto.ts`, `prisma.ts`, `ai.ts`, `billing.ts` (billing-cycle math), `statementMatching.ts` (statement-to-bill matching/scoring), `mail.ts` (Resend integration), `errors.ts`.
